...
the user's, and the cursor visibly changes as it crosses the border.

ApplyCursorTheme reads the theme out of the desktop's own configuration and
sets the variables before GLFW starts. It is a workaround for a toolkit gap,
not a feature, and it disappears if GLFW gains cursor-shape-v1.

Config files are read directly rather than shelling out to gsettings or
kreadconfig: a cursor theme is not a good enough reason for a library to start
a subprocess.
*/

// cursorSource is one file consulted for the cursor theme.
type cursorSource struct{ path, themeKey, sizeKey string }

// cursorSources are the files consulted, in order of authority. The first file
// that yields a theme name wins.
func cursorSources(home string) []cursorSource {
	return []cursorSource{
...
// initialises (before app.New), and it never overrides a value the user
// exported themselves. Away from Linux it does nothing.
func ApplyCursorTheme() {
	...
	for _, src := range cursorSources(home) {
		name, size := readCursorConfig(src.path, src.themeKey, src.sizeKey)
		if name == "" {
			continue
		}
		_ = os.Setenv("XCURSOR_THEME", name)
