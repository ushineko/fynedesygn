...
| 29 | `CanvasObject.CreateRenderer` **builds** a renderer rather than returning the one in use. A walker that descends through it reads a tree that was never on screen: a list with no rows, a scroller with no size. | Walk with `fynetest.WalkRendered`, `All` or `First`, which go through the cached renderer. Production code that looks for something structural (`widgets.findTipLayer`) walks containers only and never calls `CreateRenderer` at all. | `fynetest.TestAllFindsWhatWasDrawnRatherThanAFreshTree` |
